...
I wrote a post on Monday about shipping a hundred commits in a week and collapsing into bed. Then I woke up Tuesday and did it again.
...
Between Tuesday and Thursday, Picket gained: a full Stripe billing system with tier enforcement, self-service signup that provisions a Neon database automatically, an onboarding wizard, a redesigned admin sidebar (twice — the first version was wrong), CSA subscription management with pickup locations and skip weeks, order invoicing from admin, custom themes per tenant, and a complete rebrand from "Flora Stora" to "Picket" across every file in the codebase.
...
**Neon** spins up a Postgres database in seconds via API. Multi-tenant architecture means every new signup needs their own database — schema pushed, seed data loaded, connection string registered. That's a provisioning script, not a support ticket. I didn't build a database provisioning system. I called an API that did it for me.

**Fly.io** handles custom domains with automatic SSL certificates. A tenant wants `mindys-mums.mypicket.app`? One API call to Fly, a DNS record on their end, and it works. I didn't configure Nginx or manage Let's Encrypt. Fly did.

**Stripe Connect** let me wire up connected accounts for marketplace payments without building payment infrastructure. Stripe handles onboarding, KYC, payouts. I built the UI around it.

**Turborepo** turned a messy migration into three clean packages — app, web, shared — with cached builds. The monorepo restructure that would've been a week of yak-shaving was a day.

**Claude** wrote the implementations. Every commit says `Co-Authored-By: Claude Opus 4.6` and that's accurate. But Claude didn't make the architectural decisions or choose the tools. I did — and the choices were easy because the tools are *that* good.
...
I've had the Picket idea — a Shopify for farmers markets and CSAs — for years. I've started it twice before. Both times I stalled out on the same things: multi-tenancy is hard, payment platform integration is hard, provisioning is hard, CI/CD for a solo developer is hard. The product vision was never the problem. The *plumbing* was the problem.
